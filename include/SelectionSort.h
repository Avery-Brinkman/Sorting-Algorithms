...
void SelectionSort(vector<int>& numbers, bool step = true) {
  ...
  for (int startIndex = 0; startIndex < numbers.size(); startIndex++) {
    ...
    for (int comparsionIndex = startIndex + 1; comparsionIndex < numbers.size();
         comparsionIndex++) {
      if (step) {
        PrintVect(numbers);
        if (startIndex != 0)
          cout << setw(4 * startIndex) << setfill('-') << '-';
        cout << setw(4 * ((minIndex - startIndex) + 1)) << setfill(' ') << '*'
             << setw((4 * (comparsionIndex + 1)) - (4 * (minIndex + 1))) << setfill(' ') << '^'
             << endl;
        cout << endl;
        (void)_getch();
      }
